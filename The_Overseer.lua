...
function Modules.OverseerCE:CreateDecompilerPanel(parent)
	...
	for i, tabName in ipairs(tabs) do
		local tabBtn = self:_createButton(
			tabContainer,
			tabName,
			UDim2.new(0, tabWidth - 2, 0, 22),
			UDim2.fromOffset(2 + (i - 1) * tabWidth, 2),
			function()
...
function Modules.OverseerCE:CreateUI()
	...
	local inspectorPanel =
		self:_createPanel(content, UDim2.fromOffset(292, 26), UDim2.fromOffset(764, 660), "Table Inspector")
	...
	local patchPanel =
		self:_createPanel(content, UDim2.fromOffset(1064, 26), UDim2.fromOffset(200, 660), "Active Patches")
	...
	local function renderDump(data)
		...
		for _, entry in ipairs(sorted) do
			...
			if filter == "" or keyStr:lower():find(filter, 1, true) then
				...
				lbl.TextColor3 = vt == "function" and C.ACCENT_BLUE
...
function Modules.OverseerCE:_RemSpyPush(entry)
	...
	if spy.PauseLog then
		return
	end
	if spy.FilterText ~= "" then
		local lower = entry.Name:lower()
		if not lower:find(spy.FilterText:lower(), 1, true) then
			return
		end
	end
...
function Modules.OverseerCE:_RemSpyHookRemote(inst)
	...
	if inst:IsA("RemoteEvent") then
		local ok, orig = pcall(function()
			return hookfunction(inst.FireServer, newcclosure and newcclosure(function(self_arg, ...)
				local args = { ... }
	...
	elseif inst:IsA("RemoteFunction") then
		local ok, orig = pcall(function()
			return hookfunction(inst.InvokeServer, newcclosure and newcclosure(function(self_arg, ...)
				local args = { ... }
				local entry = {
					Time = os.date("%H:%M:%S"),
					Tick = tick(),
...
function Modules.OverseerCE:_RemSpyAddRow(scroll, entry)
	...
	row.BackgroundColor3 = entry.Kind == "Function" and Color3.fromRGB(230, 220, 255) or Color3.fromRGB(220, 235, 255)
	row.BorderSizePixel = 0
	...
	badge.BackgroundColor3 = entry.Kind == "Function" and Color3.fromRGB(138, 43, 226) or Color3.fromRGB(0, 120, 215)
	...
	kindL.TextColor3 = entry.Kind == "Function" and Color3.fromRGB(138, 43, 226) or Color3.fromRGB(0, 80, 180)
...
function Modules.OverseerCE:CreateRemoteSpyUI(parent)
	...
	local pauseBtn = self:_createButton(toolbar, "Pause", UDim2.fromOffset(50, 20), UDim2.fromOffset(400, 4), function()
		...
		pauseBtn.BackgroundColor3 = spy.PauseLog and Color3.fromRGB(255, 220, 150) or C.BG_LIGHT
	end)
...
function Modules.OverseerCE:GetConnectionsForInstance(inst)
	...
	if not getconnections then
		return {}, "getconnections() not available on this executor"
	end
	local results = {}
	local signalNames = {
		"AncestryChanged",
...
function Modules.OverseerCE:CreateConnectionViewerUI(parent)
	...
	scanBtn.TextSize = 10
	local useSelBtn = self:_createButton(
		topBar,
		"Use Selected",
		UDim2.fromOffset(80, 20),
		UDim2.new(1, -72, 0, 4),
	...
	local function populateConns(conns)
		...
		for i, entry in ipairs(conns) do
			...
			local slp = Instance.new("UIPadding", sigL)
			slp.PaddingLeft = UDim.new(0, 3)
			...
			stateL.TextColor3 = entry.Enabled and Color3.fromRGB(0, 150, 0) or Color3.fromRGB(180, 0, 0)
			...
			local discBtn = self:_createButton(
				row,
				"Disc",
				UDim2.fromOffset(38, 16),
				UDim2.fromOffset(512, 3),
				function()
			...
			local cpFnBtn = self:_createButton(
				row,
				"Copy Fn",
				UDim2.fromOffset(48, 16),
				UDim2.fromOffset(554, 3),
				function()
...
function Modules.OverseerCE:CreateUI()
	_originalCreateUI(self)
	task.defer(function()
		if not self.State.UI or not self.State.UI.ScreenGui then
			return
		end
		...
		local function addMenuBtn(label, x)
			local btn = self:_createButton(menuBar, label, UDim2.fromOffset(69, 20), UDim2.fromOffset(x, 2), function()
				self:OpenToolWindow(label)
			end)
			btn.TextSize = 11
...
local Codecs = {}
local _B64STD = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"
local _B64URL = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_"
local function _b64Decode(data, alpha)
	alpha = alpha or _B64STD
	data = data:gsub("%s", ""):gsub("=+$", "")
	if alpha == _B64URL then
...
local function _b64Encode(data, alpha)
	alpha = alpha or _B64STD
	local out = {}
	local buf, bits = 0, 0
	for i = 1, #data do
		buf = bit32.bor(bit32.lshift(buf, 8), data:byte(i))
		bits = bits + 8
		while bits >= 6 do
			bits = bits - 6
			local idx = bit32.band(bit32.rshift(buf, bits), 0x3F) + 1
			table.insert(out, alpha:sub(idx, idx))
		end
	end
	if bits > 0 then
		local idx = bit32.band(bit32.lshift(buf, 6 - bits), 0x3F) + 1
		table.insert(out, alpha:sub(idx, idx))
	end
	while #out % 4 ~= 0 do
		table.insert(out, "=")
...
Codecs.Base64Decode = function(s)
	return _b64Decode(s, _B64STD)
end
Codecs.Base64Encode = function(s)
	return _b64Encode(s, _B64STD)
end
Codecs.Base64UrlDecode = function(s)
	return _b64Decode(s, _B64URL)
end
Codecs.Base64UrlEncode = function(s)
	return _b64Encode(s, _B64URL)
end
Codecs.HexEncode = function(s)
	local out = {}
	for i = 1, #s do
...
Codecs.HexDecode = function(s)
	s = s:gsub("%s", ""):gsub("0x", ""):gsub("\\x", "")
	if #s % 2 ~= 0 then
	...
	for i = 1, #s, 2 do
		...
		if not byte then
			return nil, "Invalid hex at pos " .. i
		end
		table.insert(out, string.char(byte))
	end
	return table.concat(out)
end
Codecs.XorDecode = function(data, key)
	if not key or key == "" then
		return nil, "XOR key required"
	end
	local keyBytes = {}
	local num = tonumber(key) or tonumber(key, 16) or (key:match("^0x(.+)") and tonumber(key:match("^0x(.+)"), 16))
	if num then
		keyBytes = { math.floor(num) % 256 }
	else
		for i = 1, #key do
			table.insert(keyBytes, key:byte(i))
		end
	end
	local out = {}
	local klen = #keyBytes
	for i = 1, #data do
		...
		table.insert(out, string.char(bit32.bxor(data:byte(i), kb)))
	end
	return table.concat(out)
end
Codecs.XorEncode = Codecs.XorDecode
Codecs.Rot13 = function(s)
	return s:gsub("[A-Za-z]", function(c)
		local base = c:byte() <= 90 and 65 or 97
		return string.char((c:byte() - base + 13) % 26 + base)
	end)
end
Codecs.Caesar = function(s, shift)
	shift = (tonumber(shift) or 13) % 26
	return s:gsub("[A-Za-z]", function(c)
		local base = c:byte() <= 90 and 65 or 97
		return string.char((c:byte() - base + shift) % 26 + base)
	end)
end
Codecs.UrlDecode = function(s)
	s = s:gsub("+", " ")
	return s:gsub("%%(%x%x)", function(hex)
		return string.char(tonumber(hex, 16))
	end)
end
Codecs.UrlEncode = function(s)
	return s:gsub("[^%w%-%.%_%~ ]", function(c)
		return string.format("%%%02X", c:byte())
	end):gsub(" ", "+")
end
Codecs.Reverse = function(s)
	return s:reverse()
end
local function _autoDetect(s)
	local function isPrintable(str)
		for i = 1, #str do
			local b = str:byte(i)
	...
	if s:match("^[0-9a-fA-F%s]+$") and #s:gsub("%s", "") % 2 == 0 and #s > 4 then
		local d, err = Codecs.HexDecode(s)
		if d and isPrintable(d) then
			return d, "Hex → ASCII"
		end
	end
	if s:match("^[A-Za-z0-9+/]+=*$") and #s % 4 == 0 then
		local ok, d = pcall(Codecs.Base64Decode, s)
		if ok and d and #d > 0 and isPrintable(d) then
			return d, "Base64 → ASCII"
		end
	end
	if s:match("^[A-Za-z0-9%-_]+=*$") then
		local ok, d = pcall(Codecs.Base64UrlDecode, s)
		if ok and d and #d > 0 and isPrintable(d) then
			return d, "Base64url → ASCII"
		end
	end
	if s:match("^[A-Za-z0-9+/]+=*$") and #s % 4 == 0 then
		local ok, mid = pcall(Codecs.Base64Decode, s)
		if ok and mid then
			if mid:match("^[0-9a-fA-F]+$") and #mid % 2 == 0 then
				local d2, _ = Codecs.HexDecode(mid)
				if d2 and isPrintable(d2) then
					return d2, "Base64 → Hex → ASCII"
				end
			end
		end
	end
	local rot = Codecs.Rot13(s)
	if isPrintable(rot) and rot ~= s then
		local lrot = rot:lower()
		if
			lrot:find("function")
			or lrot:find("local")
			or lrot:find("return")
			or lrot:find("require")
			or lrot:find("game")
			or lrot:find("the ")
			or lrot:find("and ")
		then
	...
	if s:find("%%[0-9a-fA-F][0-9a-fA-F]") then
		local d = Codecs.UrlDecode(s)
		if isPrintable(d) then
			return d, "URL decode"
		end
	end
	return nil, "Could not auto-detect encoding"
end
Codecs.AutoDetect = _autoDetect
function Modules.OverseerCE:DecodeLabScanModule(mod, xorKey)
	local results = {}
	local visited = {}
	local function scanFn(fn, path)
		local addr = tostring(fn)
		if visited[addr] then
			return
		end
		visited[addr] = true
		local getFn = getconstants or (debug and debug.getconstants)
		if not getFn then
			return
		end
		local ok, consts = pcall(getFn, fn)
		...
		for i, v in pairs(consts) do
			if type(v) == "string" and #v >= 8 then
				...
				local d, m = _autoDetect(v)
				if d then
					entry.Decoded = d
					entry.Method = m
				end
				if xorKey and xorKey ~= "" then
					local xd = Codecs.XorDecode(v, xorKey)
					if xd then
						local isPrint = true
						for ci = 1, #xd do
							local b = xd:byte(ci)
						...
						if isPrint and xd ~= v then
							entry.Decoded = xd
							entry.Method = "XOR key=" .. xorKey
						end
					end
				end
				if entry.Decoded and entry.Decoded ~= v then
					table.insert(results, entry)
				end
			end
		end
		local getP = getprotos or (debug and debug.getprotos)
		if getP then
			local pok, protos = pcall(getP, fn)
			if pok and protos then
				for pi, proto in pairs(protos) do
					if type(proto) == "function" then
						scanFn(proto, path .. "[P" .. pi .. "]")
					end
				end
			end
		end
	...
	local function scanTable(tbl, path, depth)
		if depth > 4 or visited[tbl] then
			return
		end
		visited[tbl] = true
		for k, v in pairs(tbl) do
			...
			if type(v) == "function" then
				scanFn(v, sub)
			elseif type(v) == "table" then
				scanTable(v, sub, depth + 1)
			end
		end
	end
	if type(mod) == "table" then
		scanTable(mod, "module", 0)
...
local function _runPipelineStep(data, op, key)
	if op == "Base64 Decode" then
		return Codecs.Base64Decode(data)
	elseif op == "Base64 Encode" then
		return Codecs.Base64Encode(data)
	elseif op == "Base64url Decode" then
		return Codecs.Base64UrlDecode(data)
	elseif op == "Base64url Encode" then
		return Codecs.Base64UrlEncode(data)
	elseif op == "Hex Decode" then
		local d, err = Codecs.HexDecode(data)
		return d, err
	elseif op == "Hex Encode" then
		return Codecs.HexEncode(data)
	elseif op == "XOR (key below)" then
		return Codecs.XorDecode(data, key)
	elseif op == "ROT13" then
		return Codecs.Rot13(data)
	elseif op == "Caesar (shift=key)" then
		return Codecs.Caesar(data, key)
	elseif op == "URL Decode" then
		return Codecs.UrlDecode(data)
	elseif op == "URL Encode" then
		return Codecs.UrlEncode(data)
	elseif op == "Reverse" then
		return Codecs.Reverse(data)
	end
	return data
end
local function _runPipeline(input, steps)
	local cur = input
	local log = {}
	for i, step in ipairs(steps) do
		local ok, result = pcall(_runPipelineStep, cur, step.op, step.key)
		if not ok or result == nil then
			return cur, log, string.format("Step %d [%s] failed: %s", i, step.op, tostring(result))
		end
		table.insert(log, { Step = i, Op = step.op, Key = step.key, Output = result })
		cur = result
	end
	return cur, log, nil
end
function Modules.OverseerCE:CreateDecodeLabUI(parent)
	local C = self.Config
	local tabBar = Instance.new("Frame", parent)
	tabBar.Size = UDim2.new(1, 0, 0, 24)
	tabBar.Position = UDim2.fromOffset(0, 0)
	tabBar.BackgroundColor3 = C.BG_DARK
	tabBar.BorderSizePixel = 0
	local tabPages = {}
	local activeTab = nil
	local function makeTab(name, x)
		local btn = self:_createButton(tabBar, name, UDim2.fromOffset(90, 22), UDim2.fromOffset(x, 1), function()
	...
	local tabDefs = { "Pipeline", "Quick Ops", "Auto-Detect", "Module Scan" }
	for i, tname in ipairs(tabDefs) do
		makeTab(tname, (i - 1) * 92 + 2)
	end
	local pageArea = Instance.new("Frame", parent)
	pageArea.Size = UDim2.new(1, 0, 1, -26)
	pageArea.Position = UDim2.fromOffset(0, 26)
	pageArea.BackgroundColor3 = C.BG_PANEL
	pageArea.BorderSizePixel = 0
	local function makePage(name)
		local f = Instance.new("Frame", pageArea)
	...
	local function makeInputBox(parent, yOff, placeholder, multiline)
		local box = Instance.new(multiline and "TextBox" or "TextBox", parent)
		box.Size = UDim2.new(1, -8, 0, multiline and 80 or 22)
		box.Position = UDim2.fromOffset(4, yOff)
		box.BackgroundColor3 = C.BG_WHITE
		box.Text = ""
		box.PlaceholderText = placeholder or ""
		box.TextColor3 = C.TEXT_BLACK
		...
		self:_createBorder(box, true)
		local p = Instance.new("UIPadding", box)
		p.PaddingLeft = UDim.new(0, 4)
		p.PaddingTop = UDim.new(0, 2)
		return box
	end
	local function makeLabel(parent, txt, yOff, color)
		local l = Instance.new("TextLabel", parent)
		l.Size = UDim2.new(1, -8, 0, 14)
		l.Position = UDim2.fromOffset(4, yOff)
		l.BackgroundTransparency = 1
		l.Text = txt
		l.TextColor3 = color or C.TEXT_BLACK
		l.Font = Enum.Font.SourceSansBold
		l.TextSize = 10
		l.TextXAlignment = Enum.TextXAlignment.Left
		return l
	end
	local function makeOutputScroll(parent, yOff, height)
		local scroll = Instance.new("ScrollingFrame", parent)
		scroll.Size = UDim2.new(1, -8, 0, height)
		scroll.Position = UDim2.fromOffset(4, yOff)
		...
		lbl.BackgroundTransparency = 1
		lbl.Text = ""
		lbl.TextColor3 = Color3.fromRGB(190, 240, 180)
		lbl.Font = Enum.Font.Code
		lbl.TextSize = 10
		lbl.TextXAlignment = Enum.TextXAlignment.Left
	...
	do
		local p = makePage("Pipeline")
		local steps = {}
		makeLabel(p, "Input:", 4)
		local inputBox = makeInputBox(p, 18, "Paste encoded string here...", true)
		inputBox.Size = UDim2.new(1, -8, 0, 60)
		makeLabel(p, "XOR / Caesar key (shared across all XOR/Caesar steps):", 82)
		local globalKeyBox = makeInputBox(p, 96, "e.g. 42  or  0x2A  or  myKey")
		globalKeyBox.Size = UDim2.new(0.4, -8, 0, 20)
		makeLabel(p, "Pipeline steps  (top → bottom):", 120)
		local stepsScroll = Instance.new("ScrollingFrame", p)
		stepsScroll.Size = UDim2.new(1, -8, 0, 110)
		stepsScroll.Position = UDim2.fromOffset(4, 134)
		stepsScroll.BackgroundColor3 = C.BG_WHITE
		stepsScroll.BorderSizePixel = 0
		stepsScroll.ScrollBarThickness = 8
		stepsScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
		stepsScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
		self:_createBorder(stepsScroll, true)
		local stepsLayout = Instance.new("UIListLayout", stepsScroll)
		stepsLayout.Padding = UDim.new(0, 2)
		stepsLayout.SortOrder = Enum.SortOrder.LayoutOrder
		local function refreshStepOrder()
			for i, s in ipairs(steps) do
				s.row.LayoutOrder = i
			end
		end
		local function addStep(defaultOp)
			defaultOp = defaultOp or PIPELINE_OPS[1]
			local stepData = { op = defaultOp, key = "" }
			local row = Instance.new("Frame", stepsScroll)
			row.Size = UDim2.new(1, -2, 0, 24)
			row.BackgroundColor3 = #steps % 2 == 0 and C.BG_LIGHT or C.BG_WHITE
			row.BorderSizePixel = 0
			row.LayoutOrder = #steps + 1
			stepData.row = row
			local numL = Instance.new("TextLabel", row)
			numL.Size = UDim2.fromOffset(20, 24)
			numL.Position = UDim2.fromOffset(2, 0)
			numL.BackgroundTransparency = 1
			numL.Text = tostring(#steps + 1)
			numL.TextColor3 = C.TEXT_GRAY
			numL.Font = Enum.Font.SourceSansBold
			numL.TextSize = 10
			stepData.numLabel = numL
			local opBtn = self:_createButton(
				row,
				stepData.op,
				UDim2.fromOffset(160, 20),
				UDim2.fromOffset(24, 2),
				function()
					local cur = stepData.op
					local idx = 1
					for i, o in ipairs(PIPELINE_OPS) do
						if o == cur then
							idx = i
							break
						end
					end
					idx = (idx % #PIPELINE_OPS) + 1
					stepData.op = PIPELINE_OPS[idx]
					opBtn.Text = stepData.op
				end
			)
			opBtn.TextSize = 9
			opBtn.TextXAlignment = Enum.TextXAlignment.Left
			opBtn.BackgroundColor3 = Color3.fromRGB(200, 220, 255)
			local opp = Instance.new("UIPadding", opBtn)
			opp.PaddingLeft = UDim.new(0, 3)
			local delBtn = self:_createButton(row, "✕", UDim2.fromOffset(20, 20), UDim2.new(1, -22, 0, 2), function()
				row:Destroy()
				for i, s in ipairs(steps) do
					if s == stepData then
		...
		btnRow.Position = UDim2.fromOffset(4, 248)
		btnRow.BackgroundTransparency = 1
		local addBtn = self:_createButton(
			btnRow,
			"+ Add Step",
			UDim2.fromOffset(90, 22),
			UDim2.fromOffset(0, 0),
		...
		addBtn.TextSize = 10
		local presetB64 = self:_createButton(
			btnRow,
			"Preset: B64",
			UDim2.fromOffset(80, 22),
			UDim2.fromOffset(94, 0),
			function()
				for _, s in ipairs(steps) do
					s.row:Destroy()
				end
				steps = {}
				addStep("Base64 Decode")
			end
		)
		presetB64.TextSize = 9
		local presetXorB64 = self:_createButton(
			btnRow,
			"Preset: XOR→B64",
			UDim2.fromOffset(110, 22),
			UDim2.fromOffset(178, 0),
			function()
				for _, s in ipairs(steps) do
					s.row:Destroy()
				end
				steps = {}
				addStep("XOR (key below)")
				addStep("Base64 Decode")
			end
		)
		presetXorB64.TextSize = 9
		local presetB64Xor = self:_createButton(
			btnRow,
			"Preset: B64→XOR",
			UDim2.fromOffset(110, 22),
			UDim2.fromOffset(292, 0),
			function()
				for _, s in ipairs(steps) do
					s.row:Destroy()
				end
		...
		presetB64Xor.TextSize = 9
		local clearBtn = self:_createButton(
			btnRow,
			"Clear Steps",
			UDim2.fromOffset(80, 22),
			UDim2.fromOffset(406, 0),
			function()
				for _, s in ipairs(steps) do
					s.row:Destroy()
				end
		...
		clearBtn.TextSize = 9
		makeLabel(p, "Output:", 276)
		local outScroll, outLabel = makeOutputScroll(p, 290, 62)
		local runBtn = self:_createButton(
			p,
			"▶  Run Pipeline",
			UDim2.fromOffset(130, 22),
			UDim2.new(1, -138, 0, 276),
			function()
				local input = inputBox.Text
				if input == "" then
					outLabel.Text = "-- no input"
					return
				end
				if #steps == 0 then
					outLabel.Text = "-- add at least one step"
					return
				end
				local key = globalKeyBox.Text
				local stepList = {}
				for _, s in ipairs(steps) do
					table.insert(stepList, { op = s.op, key = key })
				end
				local result, log, err = _runPipeline(input, stepList)
				local lines = {}
				for _, entry in ipairs(log) do
					table.insert(
						...
						string.format(
							...
							entry.Step,
							entry.Op,
							tostring(entry.Output):sub(1, 120) .. (#tostring(entry.Output) > 120 and "…" or "")
						)
					)
				end
				if err then
				...
				table.insert(lines, "")
				table.insert(lines, "── Final Output ──")
				table.insert(lines, tostring(result))
				outLabel.Text = table.concat(lines, "\n")
			end
		)
		runBtn.TextSize = 10
		runBtn.BackgroundColor3 = Color3.fromRGB(180, 230, 180)
		local copyBtn = self:_createButton(
			p,
			"Copy Output",
			UDim2.fromOffset(90, 22),
			UDim2.new(1, -232, 0, 276),
			function()
				local txt = outLabel.Text
				if txt == "" then
					return
				end
				local final = txt:match("── Final Output ──\n(.+)$") or txt
				self:_setClipboard(final)
				self:_showNotification("Output copied!", "success")
			end
		)
		copyBtn.TextSize = 10
		addStep("Base64 Decode")
	...
	do
		local p = makePage("Quick Ops")
		makeLabel(p, "Input:", 4)
		local inputBox = makeInputBox(p, 18, "Paste string here...", true)
		inputBox.Size = UDim2.new(1, -8, 0, 70)
		makeLabel(p, "XOR / Caesar key:", 92)
		local keyBox = makeInputBox(p, 106, "42  or  0x2A  or  myKey")
		keyBox.Size = UDim2.new(0.4, -8, 0, 20)
		local btnGrid = Instance.new("Frame", p)
		btnGrid.Size = UDim2.new(1, -8, 0, 52)
		btnGrid.Position = UDim2.fromOffset(4, 130)
		btnGrid.BackgroundTransparency = 1
		local outScroll, outLabel = makeOutputScroll(p, 186, 120)
		local function runOp(opName)
			local input = inputBox.Text
			if input == "" then
				outLabel.Text = "-- no input"
				return
			...
			local ok, result = pcall(_runPipelineStep, input, opName, key)
			if ok and result then
				outLabel.Text = tostring(result)
			else
				outLabel.Text = "Error: " .. tostring(result)
			end
		end
		local opDefs = {
			{ "B64 Dec", "Base64 Decode" },
			{ "B64 Enc", "Base64 Encode" },
			{ "B64url Dec", "Base64url Decode" },
			{ "B64url Enc", "Base64url Encode" },
			{ "Hex Dec", "Hex Decode" },
			{ "Hex Enc", "Hex Encode" },
			{ "XOR", "XOR (key below)" },
			{ "ROT13", "ROT13" },
			{ "Caesar", "Caesar (shift=key)" },
			{ "URL Dec", "URL Decode" },
			{ "URL Enc", "URL Encode" },
			{ "Reverse", "Reverse" },
		}
		for i, def in ipairs(opDefs) do
			local col = ((i - 1) % 6)
			local row = math.floor((i - 1) / 6)
			local btn = self:_createButton(
				btnGrid,
				def[1],
				UDim2.fromOffset(76, 22),
				UDim2.fromOffset(col * 78 + 0, row * 26),
				function()
					runOp(def[2])
				end
			)
		...
		local copyBtn = self:_createButton(
			...
			UDim2.new(1, -98, 0, 313),
			function()
				if outLabel.Text ~= "" then
					self:_setClipboard(outLabel.Text)
					self:_showNotification("Copied!", "success")
				end
			end
		)
		copyBtn.TextSize = 9
	...
	do
		local p = makePage("Auto-Detect")
		makeLabel(p, "Paste an encoded string — Overseer will try to figure out what it is:", 4)
		local inputBox = makeInputBox(p, 18, "Paste encoded string...", true)
		inputBox.Size = UDim2.new(1, -8, 0, 80)
		makeLabel(p, "XOR key to also attempt (optional):", 102)
		local xorKeyBox = makeInputBox(p, 116, "leave blank to skip XOR attempts")
		xorKeyBox.Size = UDim2.new(0.45, -8, 0, 20)
		local outScroll, outLabel = makeOutputScroll(p, 144, 140)
		local detectBtn = self:_createButton(
			p,
			"Auto-Detect & Decode",
			UDim2.fromOffset(150, 22),
			UDim2.fromOffset(4, 140 + 148),
			function()
				local input = inputBox.Text
				if input == "" then
					outLabel.Text = "-- no input"
				...
				local xorKey = xorKeyBox.Text
				local lines = {}
				local d, m = _autoDetect(input)
				if d then
					table.insert(lines, "✓ Method : " .. m)
					table.insert(lines, "")
					table.insert(lines, d)
				else
					table.insert(lines, "✗ Auto-detect: " .. m)
				end
				if not d and xorKey == "" then
					table.insert(lines, "")
					table.insert(lines, "── XOR Brute Force (bytes 1–255) ──")
					local hits = 0
					for key = 1, 255 do
						local xd = Codecs.XorDecode(input, tostring(key))
						if xd then
							...
							if printable and xd ~= input then
								local uniq = {}
								for ci = 1, #xd do
									uniq[xd:byte(ci)] = true
								end
								local ucount = 0
								for _ in pairs(uniq) do
									ucount += 1
								end
								if ucount < math.max(10, #xd * 0.5) then
									table.insert(lines, string.format("  key=0x%02X (%d): %s", key, key, xd:sub(1, 80)))
									hits += 1
									if hits >= 8 then
										table.insert(lines, "  ... (limited to 8 candidates)")
										break
									end
								end
							end
					...
					if hits == 0 then
						table.insert(lines, "  No readable XOR candidates found")
					end
				elseif xorKey ~= "" then
					local xd, xe = Codecs.XorDecode(input, xorKey)
					if xd then
						table.insert(lines, "")
						table.insert(lines, "── XOR key=" .. xorKey .. " ──")
						table.insert(lines, xd)
						local b64ok, b64d = pcall(Codecs.Base64Decode, xd)
						if b64ok and b64d and #b64d > 0 then
							table.insert(lines, "")
							table.insert(lines, "── XOR → Base64 decode ──")
							table.insert(lines, b64d)
						end
					else
						table.insert(lines, "XOR error: " .. tostring(xe))
					end
				end
				outLabel.Text = table.concat(lines, "\n")
			end
		)
		detectBtn.TextSize = 10
		detectBtn.BackgroundColor3 = Color3.fromRGB(180, 230, 180)
		local copyBtn = self:_createButton(
			p,
			"Copy Output",
			UDim2.fromOffset(90, 22),
			UDim2.fromOffset(158, 140 + 148),
			function()
				if outLabel.Text ~= "" then
					self:_setClipboard(outLabel.Text)
					self:_showNotification("Copied!", "success")
	...
	do
		local p = makePage("Module Scan")
		makeLabel(p, "Scans the selected module's string constants and auto-decodes them.", 4)
		makeLabel(p, "Also attempts XOR with the key below if provided:", 18)
		local xorKeyBox = makeInputBox(p, 32, "XOR key  (optional: 42  /  0x2A  /  myKey)")
		xorKeyBox.Size = UDim2.new(0.5, -8, 0, 20)
		local selLabel = Instance.new("TextLabel", p)
		selLabel.Size = UDim2.new(1, -8, 0, 14)
		selLabel.Position = UDim2.fromOffset(4, 56)
		selLabel.BackgroundTransparency = 1
		selLabel.Text = "Selected module: (none)"
		selLabel.TextColor3 = C.TEXT_GRAY
		selLabel.Font = Enum.Font.SourceSansItalic
		selLabel.TextSize = 10
		selLabel.TextXAlignment = Enum.TextXAlignment.Left
		local countLabel = Instance.new("TextLabel", p)
		countLabel.Size = UDim2.fromOffset(150, 14)
		countLabel.Position = UDim2.fromOffset(4, 72)
		countLabel.BackgroundTransparency = 1
		countLabel.Text = "Results: 0"
		countLabel.TextColor3 = C.TEXT_GRAY
		countLabel.Font = Enum.Font.SourceSans
		countLabel.TextSize = 10
		countLabel.TextXAlignment = Enum.TextXAlignment.Left
		local resultScroll = Instance.new("ScrollingFrame", p)
		resultScroll.Size = UDim2.new(1, -8, 1, -116)
		resultScroll.Position = UDim2.fromOffset(4, 90)
		resultScroll.BackgroundColor3 = C.BG_WHITE
		resultScroll.BorderSizePixel = 0
		resultScroll.ScrollBarThickness = 10
		resultScroll.ScrollBarImageColor3 = C.BG_DARK
		resultScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
		resultScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
		self:_createBorder(resultScroll, true)
		Instance.new("UIListLayout", resultScroll).Padding = UDim.new(0, 1)
		local function populateResults(results)
			...
			countLabel.Text = "Results: " .. #results
			if #results == 0 then
				local emptyL = Instance.new("TextLabel", resultScroll)
				emptyL.Size = UDim2.new(1, 0, 0, 30)
				emptyL.BackgroundTransparency = 1
				emptyL.Text = "No decodable strings found"
				emptyL.TextColor3 = C.TEXT_GRAY
				emptyL.Font = Enum.Font.SourceSansItalic
				emptyL.TextSize = 11
			...
			for i, entry in ipairs(results) do
				...
				row.BorderSizePixel = 0
				self:_createBorder(row, true)
				local methodL = Instance.new("TextLabel", row)
				methodL.Size = UDim2.fromOffset(130, 14)
				methodL.Position = UDim2.fromOffset(4, 2)
				methodL.BackgroundTransparency = 1
				methodL.Text = "[" .. (entry.Method or "?") .. "]"
				methodL.TextColor3 = Color3.fromRGB(0, 100, 200)
				methodL.Font = Enum.Font.SourceSansBold
				methodL.TextSize = 9
				methodL.TextXAlignment = Enum.TextXAlignment.Left
				local pathL = Instance.new("TextLabel", row)
				pathL.Size = UDim2.new(1, -200, 0, 14)
				pathL.Position = UDim2.fromOffset(136, 2)
				pathL.BackgroundTransparency = 1
				pathL.Text = entry.Path .. "  K[" .. entry.Index .. "]"
				pathL.TextColor3 = C.TEXT_GRAY
				pathL.Font = Enum.Font.Code
				pathL.TextSize = 9
				pathL.TextXAlignment = Enum.TextXAlignment.Left
				pathL.TextTruncate = Enum.TextTruncate.AtEnd
				local rawL = Instance.new("TextLabel", row)
				rawL.Size = UDim2.new(1, -140, 0, 14)
				rawL.Position = UDim2.fromOffset(4, 16)
				rawL.BackgroundTransparency = 1
				rawL.Text = "raw: " .. entry.Raw:sub(1, 100) .. (#entry.Raw > 100 and "…" or "")
				rawL.TextColor3 = Color3.fromRGB(130, 130, 130)
				rawL.Font = Enum.Font.Code
				rawL.TextSize = 9
				rawL.TextXAlignment = Enum.TextXAlignment.Left
				rawL.TextTruncate = Enum.TextTruncate.AtEnd
				local decL = Instance.new("TextLabel", row)
				decL.Size = UDim2.new(1, -140, 0, 14)
				decL.Position = UDim2.fromOffset(4, 30)
				decL.BackgroundTransparency = 1
				decL.Text = "dec: "
					.. (entry.Decoded or ""):sub(1, 100)
					.. (#(entry.Decoded or "") > 100 and "…" or "")
				decL.TextColor3 = Color3.fromRGB(0, 140, 60)
				decL.Font = Enum.Font.Code
				decL.TextSize = 9
				decL.TextXAlignment = Enum.TextXAlignment.Left
				decL.TextTruncate = Enum.TextTruncate.AtEnd
				local cpBtn = self:_createButton(
					row,
					"Copy Dec",
					UDim2.fromOffset(64, 20),
					UDim2.new(1, -68, 0, 2),
					function()
						self:_setClipboard(entry.Decoded or "")
						self:_showNotification("Decoded value copied!", "success")
					end
				)
				cpBtn.TextSize = 8
				local cpRawBtn = self:_createButton(
					row,
					"Copy Raw",
					UDim2.fromOffset(64, 20),
					UDim2.new(1, -68, 0, 24),
					function()
						self:_setClipboard(entry.Raw)
						self:_showNotification("Raw value copied!", "success")
					end
				)
				cpRawBtn.TextSize = 8
			end
		...
		local scanBtn = self:_createButton(
			p,
			"Scan Selected Module",
			UDim2.fromOffset(160, 22),
			UDim2.fromOffset(4, 66),
			function()
				if not self.State.SelectedModule then
					self:_showNotification("Select a module from the Module List first", "warning")
					return
				end
				local modName = self.State.SelectedModule.Name
				selLabel.Text = "Selected module: " .. modName
				self:_showNotification("Scanning " .. modName .. "...", "info")
				task.spawn(function()
					local ok, mod = pcall(require, self.State.SelectedModule)
					if not ok then
						self:_showNotification("require() failed on " .. modName, "error")
						return
					end
					local xorKey = xorKeyBox.Text
					local results = self:DecodeLabScanModule(mod, xorKey ~= "" and xorKey or nil)
					populateResults(results)
					self:_showNotification(
						string.format("Scan complete: %d decodable string(s) in %s", #results, modName),
						#results > 0 and "success" or "info"
					)
				end)
			end
		)
		scanBtn.TextSize = 10
		scanBtn.BackgroundColor3 = Color3.fromRGB(180, 230, 180)
		local copyAllBtn = self:_createButton(
			p,
			"Copy All",
			UDim2.fromOffset(80, 22),
			UDim2.fromOffset(168, 66),
			function()
				local lines = {}
				for _, c in ipairs(resultScroll:GetChildren()) do
					if c:IsA("Frame") then
						local rawL = c:FindFirstChild("TextLabel")
						for _, lbl in ipairs(c:GetChildren()) do
							if lbl:IsA("TextLabel") then
								if lbl.Text:sub(1, 4) == "dec:" then
				...
				if #lines > 0 then
					self:_setClipboard(table.concat(lines, "\n"))
					self:_showNotification("All decoded values copied!", "success")
				end
			end
		)
		copyAllBtn.TextSize = 9
	end
	tabPages["Pipeline"].Visible = true
	activeTab = "Pipeline"
end
local _origOpenDL = Modules.OverseerCE.OpenToolWindow
function Modules.OverseerCE:OpenToolWindow(toolName)
	if toolName == "Dec Lab" then
		if self.State.UI and self.State.UI.ScreenGui then
			for _, child in ipairs(self.State.UI.ScreenGui:GetChildren()) do
				if child.Name:match("Window$") and child ~= self.State.UI.Main then
					child:Destroy()
		...
		local popup = Instance.new("Frame", self.State.UI.ScreenGui)
		popup.Name = "Dec LabWindow"
		popup.Size = UDim2.fromOffset(740, 420)
		popup.Position = UDim2.new(0.5, -370, 0.5, -210)
		popup.BackgroundColor3 = self.Config.BG_PANEL
		popup.BorderSizePixel = 0
		popup.ZIndex = 100
		self:_createBorder(popup, false)
		...
		grad.Color = ColorSequence.new({
			...
			ColorSequenceKeypoint.new(1, Color3.fromRGB(40, 160, 80)),
		})
		grad.Rotation = 90
		local titleL = Instance.new("TextLabel", titleBar)
		titleL.Size = UDim2.new(1, -50, 1, 0)
		titleL.Position = UDim2.fromOffset(4, 0)
		titleL.Text = "Decode Lab  —  Base64 · XOR · Hex · Pipeline · Auto-Detect · Module Scanner"
		titleL.TextColor3 = self.Config.BG_WHITE
		titleL.Font = Enum.Font.SourceSansBold
		titleL.TextSize = 12
		titleL.TextXAlignment = Enum.TextXAlignment.Left
		...
		contentArea.BorderSizePixel = 0
		contentArea.ZIndex = 100
		self:CreateDecodeLabUI(contentArea)
		local dragging, dragStart, startPos
		titleBar.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 then
				dragging = true
		...
		_origOpenDL(self, toolName)
	end
end
local _origCreateUIDL = Modules.OverseerCE.CreateUI
function Modules.OverseerCE:CreateUI()
	_origCreateUIDL(self)
	task.defer(function()
		if not self.State.UI or not self.State.UI.ScreenGui then
			return
		end
		local menuBar = nil
		for _, child in ipairs(self.State.UI.Main:GetDescendants()) do
			if child:IsA("Frame") and child.Size.Y.Offset == 22 then
				local hasBtn = false
				for _, c in ipairs(child:GetChildren()) do
					if c:IsA("TextButton") then
		...
		local btn = self:_createButton(
			...
			UDim2.fromOffset(maxX + 2, 2),
			function()
				self:OpenToolWindow("Dec Lab")
			end
		)
		btn.TextSize = 11
		btn.Size = UDim2.fromOffset(69, 20)
